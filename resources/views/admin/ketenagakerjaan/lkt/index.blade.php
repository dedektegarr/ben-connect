...
@section('content')
    <x-panel.panel class="w-full">
        <x-panel.panel-header title="{{ __('Data Lowongan Kerja Terdaftar') }}">
        ...
        <x-panel.panel-body class="w-full p-10 bg-gray-900 rounded-lg shadow-lg">
            ...
            <div
               ...
               <tbody>
                @foreach ($lkts as $key => $lkt)
                <tr>
                    <td class="font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        {{ $lkt["region_name"] ?? 'Data tidak tersedia' }}
                    </td>
                    <td>{{ $lkt['year'] }}</td>
                    <td>{{ $lkt['male'] }}</td>
                    <td>{{ $lkt['female'] }}</td>
                    <td>{{ $lkt['male'] + $lkt['female'] }}</td> {{-- Total Male + Female --}}
                </tr>
            @endforeach
