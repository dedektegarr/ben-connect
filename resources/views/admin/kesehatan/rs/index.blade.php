            ...
            <form method="GET" action="">
                <div class="flex items-center gap-4 justify-between mb-6">
                    <div class="flex gap-4 items-center w-full">
                        ...
                        {{-- Kepemilikan --}}
                        <div class="w-full">
                            <label for="ownership" class="sr-only">Kepemilikan</label>
                            <select id="ownership" name="ownership"
                                class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
                                <option value="" {{ request('ownership') == '' ? 'selected' : '' }}>Kepemilikan
                                </option>
                                @foreach ($ownerships as $item)
                                    <option value="{{ $item['hospital_ownership_name'] }}"
                                        {{ request('ownership') == $item['hospital_ownership_name'] ? 'selected' : '' }}>
                                        {{ $item['hospital_ownership_name'] }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="w-full">
                            ...
                            <select id="category" name="category"
                                class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
                                <option value="" {{ request('ownership') == '' ? 'selected' : '' }}>Semua Kategori
                                </option>
                                @foreach ($categories as $item)
                                    <option value="{{ $item['category_hospital_name'] }}"
                                        {{ request('category') == $item['category_hospital_name'] ? 'selected' : '' }}>
                                        {{ $item['category_hospital_name'] }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        {{-- Akreditasi --}}
                        <div class="w-full">
                            <label for="acreditation" class="sr-only">Akreditasi</label>
                            <select id="acreditation" name="acreditation"
                                class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
                                <option value="" {{ request('acreditation') == '' ? 'selected' : '' }}>Semua
                                    Akreditasi
                                </option>
                                @foreach ($acreditations as $item)
                                    <option value="{{ $item['hospital_acreditation_name'] }}"
                                        {{ request('acreditation') == $item['hospital_acreditation_name'] ? 'selected' : '' }}>
                                        {{ $item['hospital_acreditation_name'] }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
